...
function Update-BackupPaths {
	...
	$Username = 'Revention'
	$Password = ConvertTo-SecureString 'Astr0s' -AsPlainText -Force
    
	# Connect to SQL Server
	$connection = Connect-ToSQLServer -SqlServerInstance $SqlServerInstance -DatabaseName $DatabaseName -Username $Username -SecurePassword $Password
...
function Get-CommonSqlConnection {
	...
	$DatabaseName = 'REVENTION'
	$Username = 'Revention'
	$SecurePassword = ConvertTo-SecureString 'Astr0s' -AsPlainText -Force
	return Connect-ToSQLServer -SqlServerInstance $SqlServerInstance -DatabaseName $DatabaseName -Username $Username -SecurePassword $SecurePassword
}
...
Add-RichTextBox "$displayResults`n"
	...
	$button_SyncRecords_Click = {
		...
		# Connect to SQL Server
		$connection = Connect-ToSQLServer -SqlServerInstance $SqlServerInstance -DatabaseName $DatabaseName -Username $Username -SecurePassword $SecurePassword
